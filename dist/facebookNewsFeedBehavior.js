(function runner(debug) {
  ...
  function selectedNextElementSibling(selector, context) {
  ...
  function clickSelectedElementsNextElementSibling(selector, cntx) {
    return click(selectedNextElementSibling(selector, cntx));
  }
  ...
  const NewsFeedLoadingMore = 'div[data-testid="fbfeed_placeholder_story"]';
  const NewsFeedPlaceHolderStory =
  ...
  let behaviorClasses;
  if (debug) {
    behaviorClasses = addBehaviorStyle(
      '.wr-debug-visited {border: 6px solid #3232F1;}'
    );
  }
  const scrollDelay = 1500;
  async function* walkNewsFeed() {
    let items = xpathSnapShot(NewsFeedItemXPath);
    while (items.snapshotLength) {
      for (let i = 0; i < items.snapshotLength; ++i) {
        yield items.snapshotItem(i);
      }
      clickSelectedElementsNextElementSibling(NewsFeedPlaceHolderStory);
      items = xpathSnapShot(NewsFeedItemXPath);
      if (items.snapshotLength === 0) {
        clickSelectedElementsNextElementSibling(NewsFeedPlaceHolderStory);
        const feedPlaceHolder = qs(NewsFeedLoadingMore);
        if (feedPlaceHolder && isElemVisible(feedPlaceHolder)) {
          await waitForElementToBecomeInvisible(feedPlaceHolder);
        } else {
          await delay();
        }
        items = xpathSnapShot(NewsFeedItemXPath);
  ...
  async function* initFBNewsFeedBehaviorIterator(cliAPI) {
    const state = { videos: 0, posts: 0 };
    for await (const feedItem of walkNewsFeed()) {
      state.posts++;
      await scrollIntoViewWithDelay(feedItem, scrollDelay);
      markElemAsVisited(feedItem);
      collectOutlinksFrom(feedItem);
      const playVideo = qs(PlayVideoSelector, feedItem);
      let wait = false;
      if (playVideo) {
        wait = true;
        state.videos++;
      ...
      await maybeViewPostOrImageInTheater(feedItem);
  ...
  initRunnableBehavior({
    win: window,
    behaviorStepIterator: initFBNewsFeedBehaviorIterator({
